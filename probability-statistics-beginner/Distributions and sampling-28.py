...
temp = income[income['pop_over_25'] > 500000]
lowest_income_high_pop_county = temp['county'][temp['median_income'].idxmin()]
...
random.seed(1)
medium_sample = probability_of_one(300, 1000)
plt.hist(medium_sample, 20)
plt.ylim(0,70)
plt.xlim(0,0.4)
plt.show()
...
deviations_from_mean = numpy.abs(mean - 0.18) / large_sample_std

over_18_count = len(list(filter(lambda x: x >= 0.18, large_sample)))
...
def return_mean_medians(df):
    return df["median_income"].mean()

random.seed(1)

mean_medians = [return_mean_medians(select_random_sample(100)) for _ in range(1000)]
plt.hist(mean_medians, 20)
...
exceptions = len(list(filter(lambda x: x > 0.675, mean_ratios)))
significance_value = exceptions / len(mean_ratios)
