...
median_difference_sum = sum([value - values_median for value in values])
...
for pt in nba_stats['pts']:
    difference = pt - pt_mean
    sq_diff = difference ** 2
    pt_var += sq_diff
point_variance = pt_var / len(nba_stats['pts'])
...
mp_dev = stddev(nba_stats['mp'])
ast_dev = stddev(nba_stats['ast'])
...
distances = [length - mean for length in wing_lengths]
within_one_percentage = len([distance for distance in distances if abs(distance) < std]) / len(wing_lengths)
within_two_percentage = len([distance for distance in distances if abs(distance) < 2 * std]) / len(wing_lengths)
within_three_percentage = len([distance for distance in distances if abs(distance) < 3 * std]) / len(wing_lengths)
...
r_fta_pts, _ = pearsonr(nba_stats['fta'], nba_stats['pts'])
r_stl_pf, _ = pearsonr(nba_stats['stl'], nba_stats['pf'])
...
def cov(x_vals, y_vals):
    ...
    return sum([(x - mean_x) * (y - mean_y) for i, x in enumerate(x_vals) for j, y in enumerate(y_vals) if i == j]) / len(x_vals)
