...
y_hats = slope * wine_quality['density'] + intercept
residuals = wine_quality['quality'] - y_hats
...
se = (rss / (len(wine_quality['density']) - 2)) ** (1 / 2)

diff = (wine_quality["quality"] - predicted_y).abs()

within_one = len(wine_quality[diff < se]) / len(wine_quality['quality'])
within_two = len(wine_quality[diff < 2 * se]) / len(wine_quality['quality'])
within_three = len(wine_quality[diff < 3 * se]) / len(wine_quality['quality'])