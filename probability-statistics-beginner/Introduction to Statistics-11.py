...
plt.show()
plot2 = plt.plot(day_numbers, cars_in_parking_lot)
plt.show()
...
survey_responses = [0, 2, 3, 0, 1, 0, 0]
...
zipped = list(zip(gender, savings))

male_values = [x[1] for x in zipped if x[0] == 'male']
female_values = [x[1] for x in zipped if x[0] == 'female']

male_savings = sum(male_values) / len(male_values)
female_savings = sum(female_values) / len(female_values)
...
plt.axvline(np.mean(new_titanic_survival['age']), color='red')
plt.show()
