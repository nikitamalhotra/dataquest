...
def clean_sentence(row):
    ...
    cleaned = [s for s in row if ord(s) in good_characters]
    return "".join(cleaned)

sentences_cia['cleaned_statement'] = sentences_cia['statement'].apply(clean_sentence)
...
filtered_tokens = list(filter(lambda x: len(x) > 4, statement_tokens))
...
def find_top_two(year):
    df = sentences_cia[sentences_cia['year']==year]
    combined = ''.join(df['cleaned_statement'].tolist())
    words = combined.split(" ")
    filtered_words = list(filter(lambda x: len(x) > 4, words))
    c = Counter(filtered_words)
    return c.most_common(2)

common_2000 = find_top_two('2000')
common_2002 = find_top_two('2002')
common_2013 = find_top_two('2013')