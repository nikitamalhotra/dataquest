...
def feedforward(X, theta0, theta1):
    ...
    b = np.ones((100, 1))
    a1 = np.concatenate((b, a1), axis=1)
    a2 = sigmoid_activation(a1.T, theta1)
...
def multiplecost(x, y, theta0, theta1):
    m = x.shape[0]
    y.reshape((y.shape[0], 1))
    h = feedforward(X, theta0, theta1).T
    
    sum_term = np.multiply(y, np.log(h)) + np.multiply((1 - y), np.log(1 - h))
    
    
    return (-1 / m) * np.dot(sum_term.T, sum_term)
